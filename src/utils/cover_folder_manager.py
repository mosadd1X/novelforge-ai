...
class CoverFolderManager:
    ...
    def create_cover_folder(self, book_dir: str, title: str, 
                          series_info: Optional[Dict[str, Any]] = None) -> str:
        """
        Create a cover folder for a book or series.
        ...
        """
        if series_info:
            # For series, create folder named after series title
            series_title = series_info.get('series_title', title)
            folder_name = sanitize_filename(series_title)
            
            # Create in the series root directory
            series_root = os.path.dirname(book_dir)
            cover_folder = os.path.join(series_root, self.cover_folder_name, folder_name)
        else:
            # For single books, create folder named after book title
            folder_name = sanitize_filename(title)
            cover_folder = os.path.join(book_dir, self.cover_folder_name, folder_name)
    ...
    def get_expected_cover_path(self, book_dir: str, title: str, 
                              series_info: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        """
        cover_folder = self.create_cover_folder(book_dir, title, series_info)
    ...
    def scan_for_cover_images(self, book_dir: str, title: str, 
                            series_info: Optional[Dict[str, Any]] = None) -> List[str]:
        """
        Scan for existing cover images in the expected locations.
        ...
        # Also check the book directory directly for legacy covers
        for file in os.listdir(book_dir):
            if file.lower().startswith('cover') and os.path.isfile(os.path.join(book_dir, file)):
                file_ext = os.path.splitext(file)[1].lower()
                if file_ext in self.supported_formats:
                    legacy_path = os.path.join(book_dir, file)
                    if legacy_path not in found_images:
                        found_images.append(legacy_path)
    ...
    def copy_cover_to_expected_location(self, source_path: str, book_dir: str, title: str,
                                      series_info: Optional[Dict[str, Any]] = None) -> str:
        """
        Copy a cover image to the expected location with proper naming.
        ...
        expected_path = self.get_expected_cover_path(book_dir, title, series_info)
    ...
    def get_cover_folder_structure_info(self, book_dir: str, title: str,
                                      series_info: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Get information about the cover folder structure.
